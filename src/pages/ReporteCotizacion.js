...
const ReporteCotizaciones = () => {
    ...
    const generarPDF = () => {
      ...
      Object.keys(filtrosAplicados).forEach((key, index) => {
        doc.text(`${key}: ${filtrosAplicados[key]}`, 14, 40 + index * 10);
      });

      // Fecha y hora de creación
      const fecha = new Date().toLocaleDateString();
      const hora = new Date().toLocaleTimeString();
      doc.text(`Fecha de creación: ${fecha} - ${hora}`, 14, 40 + Object.keys(filtrosAplicados).length * 10 + 10);

      // Agregar tabla de cotizaciones
      doc.autoTable({
        startY: 60 + Object.keys(filtrosAplicados).length * 10,
