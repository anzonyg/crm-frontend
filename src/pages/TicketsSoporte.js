...
import { FaEdit, FaPlus, FaEye, FaTrash, FaFileExcel, FaFilePdf } from 'react-icons/fa';
...
const TicketsSoporte = () => {
    ...
    const exportToExcel = () => {
        const worksheet = XLSX.utils.json_to_sheet(filteredTicket);
    ...
    const exportToPDF = () => {
        const input = document.getElementById('ticketTable'); // Asegúrate que este elemento exista
        if (!input) {
            console.error('Elemento con el ID "ticketTable" no encontrado.');
            return;
        }
        ...
        html2canvas(input).then((canvas) => {
            ...
            pdf.setFontSize(18);
            pdf.text('Reporte de Tickets de Soporte', 10, 10);
